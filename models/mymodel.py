...
class My_Model(nn.Module):
    def __init__(self, in_c, n_classes):
        ...
        self.Conv2_bigger = conv_block(ch_in=64, ch_out=64)#256*256
        self.Conv3 = conv_block(ch_in=64, ch_out=64)#128*128
        self.Conv3_bigger = conv_block(ch_in=128, ch_out=128)#128*128
        self.Conv4 = conv_block(ch_in=128, ch_out=128)#64*64
        self.Conv4_bigger = conv_block(ch_in=256, ch_out=256)
        self.Conv5 = conv_block(ch_in=256, ch_out=256)
        self.Conv5_bigger = conv_block(ch_in=512, ch_out=512)
        ...
        self.Trans = fuseTransformer(get_b16_config(), img_size=32, inchannel=512, outchannel=256, ychannel=256)
        ...
        self.feature_ln = nn.LayerNorm(128)
...
#这里有可能只用一个mutihead attention不太有说服力？ ，所以我可以用分割类型的muti head attention就先分成几个模块这种吗？
       ...
       # att_feature = self.fc(att)
        ...
        x2 = self.Conv2_bigger(x2)
        x3 = self.Maxpool(x2)
        x3_1 = self.Conv3(x3)
        x3_2 = self.edgat3(x3)
        x3 = torch.cat((x3_1,x3_2),dim=1)
        x3 = self.Conv3_bigger(x3)
        x4 = self.Maxpool(x3)
        x4_1 = self.Conv4(x4)
        x4_2 = self.edgat4(x4)
        x4 = torch.cat((x4_1,x4_2),dim=1)
        x4 = self.Conv4_bigger(x4)
        x5 = self.Maxpool(x4)
        x5_1 = self.Conv5(x5)
        x5_2 = self.edgat5(x5)
        x5 = torch.cat((x5_1,x5_2),dim=1)
        x6 = self.Conv5_bigger(x5)
