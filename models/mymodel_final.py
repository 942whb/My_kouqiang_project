...
class My_Model_final(nn.Module):
    def __init__(self, in_c, n_classes):
        super(My_Model_final,self).__init__()
        ...
        self.seg_Conv1 = conv_block(ch_in=3, ch_out=32)#512*512
        self.seg_Conv2 = conv_block(ch_in=32, ch_out=32)#256*256
        self.seg_Conv3 = conv_block(ch_in=64, ch_out=64)#128*128
        self.seg_Conv4 = conv_block(ch_in=128, ch_out=128)#64*64
        self.seg_Conv5 = conv_block(ch_in=256, ch_out=16)#64*
        self.seg_edgat2 = MultiSpectralAttentionLayer(32,128,128, freq_sel_method = 'top16')
        self.seg_edgat3 = MultiSpectralAttentionLayer(64,64,64, freq_sel_method = 'top16')
        self.seg_edgat4 = MultiSpectralAttentionLayer(128,32,32, freq_sel_method = 'top16')
...
#这里有可能只用一个mutihead attention不太有说服力？ ，所以我可以用分割类型的muti head attention就先分成几个模块这种吗？
        ...
        feature = self.feature_ln(feature)
        feature = torch.cat([feature,att_feature],dim=1)
        ...
        sx1 = self.seg_Conv1(seg_input) 
        sx2 = self.Maxpool(sx1)
        sx2_1 = self.seg_Conv2(sx2)
        sx2_2 = self.seg_edgat2(sx2)
        sx2 = torch.cat((sx2_1,sx2_2),dim=1)

        sx3 = self.Maxpool(sx2)
        sx3_1 = self.seg_Conv3(sx3)
        sx3_2 = self.seg_edgat3(sx3)
        sx3 = torch.cat((sx3_1,sx3_2),dim=1)

        sx4 = self.Maxpool(sx3)
        sx4_1 = self.seg_Conv4(sx4)
        sx4_2 = self.seg_edgat4(sx4)
        sx4 = torch.cat((sx4_1,sx4_2),dim=1)
        sx5 = self.seg_Conv5(sx4)
        #print(x.shape,x1.shape,x2.shape,x3.shape,x4.shape)#[8,3,512,512] [8,32,512,512] [8,64,256,256] [8,128,128,128] [8,256,64,64]
        #sx7 = self.Trans(sx4)
        sd1 = self.pool_tofc(sx5)
        #print(x7.shape,d1.shape)#torch.Size([batch, 256, 16, 16]) torch.Size([batch, 256, 1, 1])
        sfeature = sd1.squeeze(dim=3).squeeze(dim=2)
        pad = self.liner(sfeature)
        pad = self.sig(pad)
        pad = pad - 0.5 
        out = pad + out
        out = torch.clamp(out,0,1)
        return out,sx5,self.liner.weight,pad,sfeature
