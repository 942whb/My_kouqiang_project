...
def evaluate(logits, labels, n_classes, ignore_index = -100, fast=True):
    ...
    for i in range(len(logits)):
        ...
        prob = logits[i].detach().cpu().numpy()
        target = labels[i].cpu().numpy()
    ...
    all_targets_np = all_targets_np[all_targets_np!=ignore_index]
    print('pred:',all_probs_np,'GT:',all_targets_np)
    if n_classes == 4:
        ...
        all_probs_np[all_probs_np > 0.5] = 1
        all_probs_np[all_probs_np <= 0.5] = 0
        if fast==True:
            return accuracy_score(all_targets_np, all_probs_np), f1_score(all_targets_np>0.5, all_preds_np)
        else:
